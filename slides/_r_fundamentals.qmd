...
![](img/save_meta_data.png)  

## :two: Utiliser des fonctions {.smaller}

À ne pas reproduire chez vous

![](img/save_meta_data_annote.png)  
...
## :three: Documenter son code {.smaller}

::: {.callout-tip}
## Utiliser aussi "l'auto-documentation" pour les noms de répertoire !

:negative_squared_cross_mark: pas de répertoires avec votre prénom, votre nom ou vos initiales.

:::

...au risque de se retrouver avec ça : 
![](img/prenoms_repertoires.png){height="400" fig-align="center"}
Taille totale : 7 Go
...
:::{.callout-tip}
## _Spécial conj_
Dès que vos codes font appels à des fonctions des packages "maison" tels que `pRev` ou `RConj` : ajoutez le nom du package devant la fonction.
Lors des montées de version des packages, vous retrouverez facilement les endroits dans vos codes qui peuvent être affectés.
:::
...
:arrow_right: pour retrouver facilement les appels à un package donné.
